...
9. Если настроен ровно один `TELEGRAM_TARGET_CHAT_ID` или один chat id в `TELEGRAM_TARGET_CHAT_IDS`, бот может выдать первую ссылку привязки даже без уже привязанного участника в voice-канале.
10. Каждая кнопка ведет на одноразовый Deep Link вида:

```text
https://t.me/<TELEGRAM_BOT_USERNAME>?start=auth_<INVITE_TOKEN>
```

11. Пользователь открывает ссылку, запускает Telegram-бота, бот извлекает invite token и определяет, к какой беседе относится попытка привязки.
12. Если пользователь не состоит в этой беседе, привязка отклоняется.
13. Если пользователь состоит в этой беседе, сохраняется связка `discord_id + telegram_chat_id -> telegram_id`.
